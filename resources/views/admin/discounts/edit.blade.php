    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.discounts.update", [$discount->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('customer') ? 'is-invalid' : '' }}" name="customer_id" id="customer_id" required>
                    @foreach($customers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('customer_id') ? old('customer_id') : $discount->customer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('customer_date_of_birth') ? 'is-invalid' : '' }}" name="customer_date_of_birth_id" id="customer_date_of_birth_id">
                    @foreach($customer_date_of_births as $id => $entry)
                        <option value="{{ $id }}" {{ (old('customer_date_of_birth_id') ? old('customer_date_of_birth_id') : $discount->customer_date_of_birth->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('token') ? 'is-invalid' : '' }}" name="token" id="token">{{ old('token', $discount->token) }}</textarea>
                @if($errors->has('token'))
                    <div class="invalid-feedback">
                        {{ $errors->first('token') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('discount') ? 'is-invalid' : '' }}" type="number" name="discount" id="discount" value="{{ old('discount', $discount->discount) }}" step="1" required>
